...
% add your corrected code from task 1
...
%%% Revise the following %%%
% mix with carrier
wave_r1 = rx_wave.*sin(2*pi*Fc*t);   %<<= error
[msg_r1, lp_wave_r1] = rx_BPSK(rx_wave, Fs, Fc, SPB, Fcutoff);
% % % % Do not change the code below % % % %

% low pass filtering to remove signals outside bandwidth
lp_wave_r1=lowpass(wave_r1, Fs, Fcutoff);
% waveform to message 1
msg_r1 = waveform2text(lp_wave_r1, SPB);

% % % % % % % % % % % % % %
% Receiver for Message 2 %
% % % % % % % % % % % % % %
wave_r2 = rx_wave.*cos(2*pi*Fc2*t);

[msg_r2, lp_wave_r2] = rx_BPSK(rx_wave, Fs, Fc2, SPB, Fcutoff);
...
figure(1);clf;
subplot(3,1,1)
plotAmplitudeSpectrum(rx_wave,Fs,'Amplitude Spectrum of the Received Waveform (1 and 2)');
subplot(3,1,2)
plotAmplitudeSpectrum(wave_r1,Fs,'Amplitude Spectrum of the Received Signal 1 before lowpass filter');
subplot(3,1,3)
plotAmplitudeSpectrum(wave_r2,Fs,'Amplitude Spectrum of the Received Signal 2 before lowpass filter');

figure(2); clf;
subplot(2,1,1);
plotAmplitudeSpectrum(lp_wave_r1,Fs,'Amplitude Spectrum of the Received Signal 1');
subplot(2,1,2);
plotAmplitudeSpectrum(lp_wave_r2,Fs,'Amplitude Spectrum of the Received Signal 2');


% generate eye diagram plots
figure(3); clf;
subplot(2,1,1);
eye_diagram(lp_wave_r1,1501,SPB);
title('Received waveform (MSG1)')
subplot(2,1,2);
eye_diagram(lp_wave_r2,1501,SPB);
